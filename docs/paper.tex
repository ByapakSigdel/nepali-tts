...
training progress. A planned extension (Section~\ref{sec:future}) additionally accepts \emph{romanized}
(Latin-script) Nepali and code-mixed Nepali-English input via transliteration to Devanagari.
...
hunuhuncha tapai}''. We are adding a frontend that transliterates informal romanized Nepali to Devanagari and
handles code-mixed Nepali-English tokens, so users need not type Devanagari. Beyond input, producing a
genuinely natural, casually code-switched voice (mirroring real Nepali-English speech) is a modeling and data
problem: it requires code-mixed training data and consistent phoneme handling for embedded English words ---
a longer-term research phase.
